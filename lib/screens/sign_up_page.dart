...
// Simple AppUser model (add this or import from a models file if it exists)
class AppUser {
  final String uid;
  final String email;
  final String role;
...
class _SignupScreenState extends State<SignupScreen> {
  ...
  final _adminCodeController = TextEditingController(); // Added for admin code input
  ...
  final bool _isAdminSignup = true; // Assuming admin signup; set to false for student flow if needed

  // Add this new method to check admin code
  Future<bool> _verifyAdminCode(String code) async {
    // In a real app, you would check this against a database
    // For demo purposes, we'll use a hardcoded code
    const validAdminCode = 'ADMIN12345678'; // 12-character code
    return code == validAdminCode;
  }

  // Updated signUp method to include admin code verification
  Future<void> signUp() async {
    if (!_formKey.currentState!.validate()) return;

    // Show admin code dialog if this is admin signup
    if (_isAdminSignup) {
      final adminCode = await showDialog<String>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: TextFormField(
            controller: _adminCodeController, // Use the controller here
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.pop(context, _adminCodeController.text);
              },
              child: Text('Verify'),
            ),
          ],
      ...
      if (adminCode == null || adminCode.isEmpty) return;

      final isValid = await _verifyAdminCode(adminCode);
      if (!isValid) {
        setState(() {
    ...
    try {
      ...
      final user = AppUser(
        ...
        email: _emailController.text.trim(),
        role: _isAdminSignup ? 'admin' : 'student',
  ...
  Future<void> signUpWithGoogle() async {
    ...
    try {
      // Show admin code dialog for Google signup
      final adminCode = await showDialog<String>(
        context: context,
        builder: (context) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('Please enter the admin code to create an admin account with Google.'),
              SizedBox(height: 16),
              TextFormField(
      ...
      if (userCredential?.user != null) {
        setState(() {
          _successMessage = 'Admin account created successfully with Google!';
        });
  ...
  void dispose() {
    ...
    _adminCodeController.dispose(); // Dispose the new controller
    super.dispose();
  }
}
